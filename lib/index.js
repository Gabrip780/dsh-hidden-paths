...
function resolveCandidate(raw, base) {
	...
	// `path.resolve` on both branches: on Windows this qualifies a POSIX-style
	// absolute spelling (`/srv/vault`) as `C:\srv\vault`, matching how the
	// configured roots are resolved. `path.normalize` alone left it drive-less
	// and an unrelated-looking string, so the comparison missed.
...
export function shellCandidates(command) {
	...
	// Windows path spellings: quotes removed but BACKSLASHES KEPT, so a command
	// like `cat C:\Users\me\vault\f` still yields one path token. The normalised
	// pass above strips backslashes — correct for POSIX escaping — and would
	// otherwise turn that into `C:Usersmevaultf`, losing the separators and the
	// protection with them.
	//
	// GATED ON THE HOST SEPARATOR. On POSIX a backslash is an escape, not a
	// separator, so a token such as `secrets\b` (a grep pattern) or `a\.b` would be
	// read as a path and wrongly denied. Off Windows this pass is skipped and the
	// previous behaviour is preserved exactly.
	if (path.sep === "\\") {
		for (const token of command.replace(/["']/g, "").split(/[\s;|&()<>`]+/)) {
			const value = token.trim();
			if (value === "" || !value.includes("\\")) continue;
			found.push({ value, quoted: false, obfuscated: false });
		}
	}
	const seen = new Set();
...
export function inspectConfig(config = {}) {
	...
	// are caught. The test is `path.parse(p).root === p`: that is `/` on POSIX and
	// `C:\` on Windows, so comparing against `path.sep` alone missed the Windows
	// drive root and the warning never fired there.
	const normalised = raw.map((entry) => path.resolve(entry));
	if (normalised.some((entry) => path.parse(entry).root === entry)) {
		issues.push(
...
/**
 * Compile a hidden path to a regex fragment matching it with EITHER separator at
 * every boundary, so `C:\srv\vault`, `C:/srv/vault` and mixed spellings all match.
 * Enumerating spellings would be exponential; one fragment covers them all.
 */
function pathAlternation(absolute) {
	return absolute.split(/[\\/]+/).map(escapeRegExp).join("[/\\\\]");
}
...
export function createHiddenPathFilter(config = {}) {
	...
	const rules = hidden.map((absolute) => ({ absolute, pattern: pathAlternation(absolute), base: basename(absolute) }));
	const isLongListing = /^[-dlbcps][rwxStTs-]{9}[.+@]?\s/;
	return (text) => {
		if (typeof text !== "string" || text === "") return null;
		let out = text;
		let count = 0;
		for (const { absolute, pattern, base } of rules) {
